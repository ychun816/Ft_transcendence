...
class Pong
{
	...
	start(): void {
		...
		// S'assurer que le container HTML est prêt
		this.initializeHTMLGameContainer();
		...
		try {
			...
			if (response.ok) {
				...
				if (stats.ok) {
					...
					console.log('Stats retrieved:', statsdata);
				}
			} else {
				console.error('Failed to send game stats');
			}
		} catch (error) {
    ...
    restart(): void
    {
		...
		// Réinitialiser le container HTML
		this.initializeHTMLGameContainer();
	...
	// NOUVELLE FONCTION DRAW AVEC TAILWIND CSS POUR 4 JOUEURS
	draw(interpolation: number): void {
		...
		gameContainer.innerHTML = `
			<div class="absolute inset-0 w-[800px] h-[600px] bg-gradient-to-b from-gray-900 to-gray-800">
				...
				<!-- Raquette P1 (gauche haut) -->
				<div class="absolute bg-gradient-to-b from-cyan-400 to-cyan-800 border border-cyan-400"
					 style="left: 30px; top: ${this.paddle.paddles.p1_y}px; width: 10px; height: 78px; box-shadow: 0 0 20px rgb(34 211 238);">
				</div>

				<!-- Raquette P2 (gauche bas) -->
				<div class="absolute bg-gradient-to-b from-cyan-400 to-cyan-800 border border-cyan-400"
					 style="left: 30px; top: ${this.paddle.paddles.p2_y}px; width: 10px; height: 78px; box-shadow: 0 0 20px rgb(34 211 238);">
				</div>

				<!-- Raquette P3 (droite haut) -->
				<div class="absolute bg-gradient-to-b from-purple-400 to-purple-800 border border-purple-400"
					 style="left: 760px; top: ${this.paddle.paddles.p3_y}px; width: 10px; height: 78px; box-shadow: 0 0 20px rgb(147 51 234);">
				</div>

				<!-- Raquette P4 (droite bas) -->
				<div class="absolute bg-gradient-to-b from-purple-400 to-purple-800 border border-purple-400"
					 style="left: 760px; top: ${this.paddle.paddles.p4_y}px; width: 10px; height: 78px; box-shadow: 0 0 20px rgb(147 51 234);">
				</div>

				<!-- Balle (20x20px de diamètre, donc rayon 10px comme l'original) -->
				...
				<!-- Effets de scanlines pour l'ambiance rétro -->
				...
				<!-- Indicateurs de coin (comme sur le canvas original) -->
